...
function PerformancePanel({ title, period, onPeriodChange, items, aggregateSeries, selectedItems, onToggleItem, collapsed, onToggleCollapsed, pickerLabel, pickerMode = "side", pickerColumnLabel = "계좌", valuationDailyRate, itemDailyRates }: { title: string; period: ReportPeriod; onPeriodChange: (period: ReportPeriod) => void; items: TrendSeries[]; aggregateSeries: TrendSeries; selectedItems: string[]; onToggleItem: (id: string) => void; collapsed: boolean; onToggleCollapsed: () => void; pickerLabel: string; pickerMode?: "side" | "below"; pickerColumnLabel?: string; valuationDailyRate?: number | null; itemDailyRates?: Map<string, number | null> }) {
  ...
  const firstSnapshot = aggregateSeries.snapshots[0];
  const lastSnapshot = aggregateSeries.snapshots.at(-1);
  const periodComparison = aggregateMode && firstSnapshot && lastSnapshot && firstSnapshot.total > 0
    ? { rate: (lastSnapshot.total / firstSnapshot.total - 1) * 100, firstAmount: firstSnapshot.total, lastAmount: lastSnapshot.total, profit: lastSnapshot.total - firstSnapshot.total, firstDate: firstSnapshot.date, lastDate: lastSnapshot.date }
    : null;
  ...
  return <section className="content-grid trend-content-grid"><article className="panel performance-panel"><header className="trend-panel-header"><div className="trend-heading"><div><h2>{title}</h2><p>{aggregateMode ? periodComparison ? `${periodComparison.firstDate.slice(5).replace("-", ".")} → ${periodComparison.lastDate.slice(5).replace("-", ".")} · 전체 ${items.length}개 항목 합산` : "비교 기준 생성 중" : `선택 항목 ${visibleItems.length}개 비교`}</p></div>{periodComparison && <div className="trend-headline-metrics"><div><span>기간 수익률</span><strong className={periodComparison.rate < 0 ? "negative" : "positive"}>{percent(periodComparison.rate)}</strong></div><div><span>통합 평가금액</span><strong className="valuation-amount">{won.format(periodComparison.lastAmount)}<small>{won.format(periodComparison.firstAmount)} → {won.format(periodComparison.lastAmount)}</small></strong></div><div><span>기간 평가손익</span><strong className={periodComparison.profit < 0 ? "negative" : "positive"}>{periodComparison.profit >= 0 ? "+" : ""}{won.format(periodComparison.profit)}</strong></div></div>}</div></header><div className="trend-panel-body"><div className={`trend-workspace ${pickerMode === "below" ? "without-selection" : ""}`}><div className="trend-chart-column"><div className="trend-series-list"><TrendChart series={chartSeries} stackSeries={aggregateMode ? items : visibleItems} showLegend={false} /></div><div className="periods" role="tablist">{reports.map(item => <button key={item} role="tab" aria-selected={period === item} className={period === item ? "selected" : ""} onClick={() => onPeriodChange(item)}>{item}</button>)}</div></div>{pickerMode === "side" && <aside className="trend-selection-panel" aria-label={pickerLabel}><div className="trend-checklist">{items.map(item => { const { rate, profit } = itemPerformance(item); const tone = rate === null ? "" : rate >= 0 ? "positive" : "negative"; return <button key={item.id} className={selectedItems.includes(item.id) ? "selected" : ""} onClick={() => onToggleItem(item.id)} aria-pressed={selectedItems.includes(item.id)}><i style={{ background: item.color }} /><span>{item.name}</span><strong className={tone}>{rate === null || profit === null ? "-" : <>{percent(rate)}<small>{profit >= 0 ? "+" : ""}{won.format(profit)}</small></>}</strong></button>; })}</div></aside>}</div>{pickerMode === "below" && <button className="trend-list-toggle" onClick={onToggleCollapsed} aria-expanded={!collapsed}>{collapsed ? `${pickerColumnLabel} 목록 펼치기` : `${pickerColumnLabel} 목록 접기`}<span aria-hidden="true">⌃</span></button>}{!collapsed && accountPicker}</div></article></section>;
}
  ...
  useEffect(() => {
    ...
    setProfitPeaks(current => {
      ...
      domesticTopThree.items.forEach(item => {
        ...
        if (!next[key] || item.value > next[key].value) next[key] = { value: item.value, date };
      });
      return next;
    });
