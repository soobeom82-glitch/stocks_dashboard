...
function PerformancePanel({ title, period, onPeriodChange, items, aggregateSeries, selectedItems, onToggleItem, collapsed, onToggleCollapsed, pickerLabel, pickerMode = "side" }: { title: string; period: ReportPeriod; onPeriodChange: (period: ReportPeriod) => void; items: TrendSeries[]; aggregateSeries: TrendSeries; selectedItems: string[]; onToggleItem: (id: string) => void; collapsed: boolean; onToggleCollapsed: () => void; pickerLabel: string; pickerMode?: "side" | "below" }) {
  ...
  const accountPicker = pickerMode === "below" && <div className="trend-account-picker" aria-label={pickerLabel}><div className="trend-account-picker-heading"><span>계좌</span><span>평가금액</span><span>수익률</span><span>평가손익</span><span>비중</span></div>{items.map(item => { const { rate, profit } = itemPerformance(item); const amount = item.snapshots.at(-1)?.total ?? 0; const tone = rate === null ? "" : rate >= 0 ? "positive" : "negative"; return <button key={item.id} className={selectedItems.includes(item.id) ? "selected" : ""} onClick={() => onToggleItem(item.id)} aria-pressed={selectedItems.includes(item.id)}><span className="trend-account-name"><i style={{ background: item.color }}>{item.iconLabel}</i><span><b>{item.name}</b><small>{item.subtitle}</small></span></span><strong>{won.format(amount)}</strong><strong className={tone}>{rate === null ? "-" : percent(rate)}</strong><strong className={tone}>{profit === null ? "-" : <>{profit >= 0 ? "+" : ""}{won.format(profit)}</>}</strong><span>{pickerTotal > 0 ? (amount / pickerTotal * 100).toFixed(1) : "0.0"}%</span></button>; })}</div>;
