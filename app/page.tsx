...
function PerformancePanel({ title, period, onPeriodChange, items, aggregateSeries, selectedItems, onToggleItem, collapsed, onToggleCollapsed, pickerLabel, showPicker = true }: { title: string; period: ReportPeriod; onPeriodChange: (period: ReportPeriod) => void; items: TrendSeries[]; aggregateSeries: TrendSeries; selectedItems: string[]; onToggleItem: (id: string) => void; collapsed: boolean; onToggleCollapsed: () => void; pickerLabel: string; showPicker?: boolean }) {
  ...
  return <section className="content-grid trend-content-grid"><article className={`panel performance-panel ${collapsed ? "collapsed" : ""}`}><header className="trend-panel-header"><div className="trend-heading"><div><h2>{title}</h2><p>{collapsed ? "접힌 상태입니다" : aggregateMode ? `전체 ${items.length}개 항목 합산` : `선택 항목 ${visibleItems.length}개 비교`}</p></div>{!collapsed && headline && <div className="trend-headline-metrics"><div><span>통합 수익률</span><strong className={headline.rate !== null && headline.rate < 0 ? "negative" : "positive"}>{headline.rate === null ? "-" : percent(headline.rate)}</strong></div><div><span>평가손익</span><strong className={headline.profit !== null && headline.profit < 0 ? "negative" : "positive"}>{headline.profit === null ? "-" : <>{headline.profit >= 0 ? "+" : ""}{won.format(headline.profit)}</>}</strong></div></div>}</div><button className="trend-collapse-button" onClick={onToggleCollapsed} aria-expanded={!collapsed}>{collapsed ? "펼치기" : "접기"}<span aria-hidden="true">⌃</span></button></header>{!collapsed && <div className="trend-panel-body"><div className={`trend-workspace ${showPicker ? "" : "without-selection"}`}><div className="trend-chart-column"><div className="trend-series-list"><TrendChart series={chartSeries} showLegend={false} /></div><div className="periods" role="tablist">{reports.map(item => <button key={item} role="tab" aria-selected={period === item} className={period === item ? "selected" : ""} onClick={() => onPeriodChange(item)}>{item}</button>)}</div></div>{showPicker && <aside className="trend-selection-panel" aria-label={pickerLabel}><div className="trend-checklist">{items.map(item => { const { rate, profit } = itemPerformance(item); const tone = rate === null ? "" : rate >= 0 ? "positive" : "negative"; return <button key={item.id} className={selectedItems.includes(item.id) ? "selected" : ""} onClick={() => onToggleItem(item.id)} aria-pressed={selectedItems.includes(item.id)}><i style={{ background: item.color }} /><span>{item.name}</span><strong className={tone}>{rate === null || profit === null ? "-" : <>{percent(rate)}<small>{profit >= 0 ? "+" : ""}{won.format(profit)}</small></>}</strong></button>; })}</div></aside>}</div></div>}</article></section>;
}
...
export default function Home() {
  ...
  return <main>
    ...
    <PerformancePanel title="통합 자산 추이" period={period} onPeriodChange={setPeriod} items={trendItems} aggregateSeries={aggregateAccountTrend} selectedItems={selectedTrendItems} onToggleItem={toggleTrendItem} collapsed={portfolioTrendCollapsed} onToggleCollapsed={() => setPortfolioTrendCollapsed(current => !current)} pickerLabel="그래프에 표시할 계좌" showPicker={false} />
    <PerformancePanel title="자산 유형별 추이" period={period} onPeriodChange={setPeriod} items={assetTrendItems} aggregateSeries={aggregateAssetTrend} selectedItems={selectedAssetTrendItems} onToggleItem={toggleAssetTrendItem} collapsed={assetTrendCollapsed} onToggleCollapsed={() => setAssetTrendCollapsed(current => !current)} pickerLabel="그래프에 표시할 자산 유형" />
    ...
    <section className="accounts-section"><div className="panel-head"><div><p className="eyebrow">ACCOUNTS</p><h2>계좌별 자산</h2></div></div><p className="account-hint">평가금액이 큰 계좌부터 표시됩니다. 계좌를 클릭하면 보유자산 상세를 펼치고 통합 자산 추이 항목에 추가합니다.</p><div className="account-table"><div className="table-heading"><span>계좌</span><span>평가금액</span><span>수익률</span><span>평가손익</span><span>비중</span></div>{accountsByValue.map(account => { const details = detailsFor(account); const profit = account.returnRate > -100 ? account.amount - account.amount / (1 + account.returnRate / 100) : 0; const accountTrendId = `account-${account.id}`; const holdingSelection = selectedHoldingTrendItems[account.id] ?? [`account-total-${account.id}`]; const toggleHoldingTrendItem = (id: string) => setSelectedHoldingTrendItems(current => { const selected = current[account.id] ?? [`account-total-${account.id}`]; return { ...current, [account.id]: selected.includes(id) ? selected.filter(item => item !== id) : [...selected, id] }; }); const toggleAccount = () => { setExpandedAccountId(current => current === account.id ? null : account.id); toggleTrendItem(accountTrendId); }; return <div className="account-item" key={account.id}><div className={`account-row ${expandedAccountId === account.id ? "expanded" : ""} ${selectedTrendItems.includes(accountTrendId) ? "trend-selected" : ""}`} role="button" tabIndex={0} onClick={toggleAccount} onKeyDown={event => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); toggleAccount(); } }}><div><span className={`account-icon ${account.color}`}>{account.type.slice(0, 1)}</span><span><b>{accountLabel(account.name)}</b><small>{account.broker}</small></span></div><strong>{won.format(account.amount)}</strong><strong className={account.returnRate >= 0 ? "positive" : "negative"}>{percent(account.returnRate)}</strong><strong className={profit >= 0 ? "positive" : "negative"}>{profit >= 0 ? "+" : ""}{won.format(profit)}</strong><div className="weight"><i><em style={{ width: `${total > 0 ? account.amount / total * 100 : 0}%` }}/></i><span>{total > 0 ? (account.amount / total * 100).toFixed(1) : "0.0"}%</span></div></div>{expandedAccountId === account.id && <AccountDetails account={account} {...details} snapshots={periodSnapshots} selectedTrendItems={holdingSelection} onToggleTrendItem={toggleHoldingTrendItem} />}</div>; })}</div></section>
  </main>;
}
