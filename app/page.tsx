...
function PerformancePanel({ title, period, onPeriodChange, items, aggregateSeries, selectedItems, onToggleItem, collapsed, onToggleCollapsed, pickerLabel }: { title: string; period: ReportPeriod; onPeriodChange: (period: ReportPeriod) => void; items: TrendSeries[]; aggregateSeries: TrendSeries; selectedItems: string[]; onToggleItem: (id: string) => void; collapsed: boolean; onToggleCollapsed: () => void; pickerLabel: string }) {
  const visibleItems = items.filter(item => selectedItems.includes(item.id));
  const allItemsSelected = items.length > 0 && visibleItems.length === items.length;
  const commonDates = visibleItems.reduce<string[] | null>((dates, item) => dates === null ? item.snapshots.map(snapshot => snapshot.date) : dates.filter(date => item.snapshots.some(snapshot => snapshot.date === date)), null) ?? [];
  const selectedAggregate: TrendSeries = { id: "selected-total", name: allItemsSelected ? aggregateSeries.name : `선택 ${visibleItems.length}개 항목 합산`, color: "#5666df", snapshots: commonDates.map(date => visibleItems.reduce<Snapshot>((sum, item) => { const snapshot = item.snapshots.find(candidate => candidate.date === date)!; return { date, total: sum.total + snapshot.total, cost: (sum.cost ?? 0) + (snapshot.cost ?? item.currentCost ?? 0) }; }, { date, total: 0, cost: 0 })) };
  const chartSeries = visibleItems.length ? [allItemsSelected ? aggregateSeries : selectedAggregate] : [];
  return <section className="content-grid trend-content-grid"><article className={`panel performance-panel ${collapsed ? "collapsed" : ""}`}><header className="trend-panel-header"><div><h2>{title}</h2><p>{collapsed ? "접힌 상태입니다" : allItemsSelected ? `전체 ${items.length}개 항목 합산 · 수익률과 평가손익` : `선택 항목 ${visibleItems.length}개 합산 · 수익률과 평가손익`}</p></div><button className="trend-collapse-button" onClick={onToggleCollapsed} aria-expanded={!collapsed}>{collapsed ? "펼치기" : "접기"}<span aria-hidden="true">⌃</span></button></header>{!collapsed && <div className="trend-panel-body"><div className="trend-workspace"><div className="trend-chart-column"><div className="trend-series-list">{chartSeries.length ? <TrendChart series={chartSeries} /> : <div className="empty-holdings">그래프로 볼 항목을 하나 이상 선택해 주세요.</div>}</div><div className="periods" role="tablist">{reports.map(item => <button key={item} role="tab" aria-selected={period === item} className={period === item ? "selected" : ""} onClick={() => onPeriodChange(item)}>{item}</button>)}</div></div><aside className="trend-selection-panel" aria-label={pickerLabel}><div><h3>표시 항목</h3><span>{visibleItems.length}개 선택</span></div><div className="trend-checklist">{items.map(item => <label key={item.id}><input type="checkbox" checked={selectedItems.includes(item.id)} onChange={() => onToggleItem(item.id)} /><i style={{ background: item.color }} /><span>{item.name}</span></label>)}</div></aside></div></div>}</article></section>;
}
...
export default function Home() {
  ...
  const [selectedTrendItems, setSelectedTrendItems] = useState<string[]>(["account-1", "account-2", "account-3", "account-4", "account-5", "account-6", "account-7", "account-8"]);
  const [selectedAssetTrendItems, setSelectedAssetTrendItems] = useState<string[]>(["asset-국내 주식", "asset-해외 주식", "asset-채권·현금성", "asset-대체자산", "asset-펀드", "asset-가상자산"]);
  ...
  return <main>
    ...
    <PerformancePanel title="통합 자산 추이" period={period} onPeriodChange={setPeriod} items={trendItems} aggregateSeries={aggregateAccountTrend} selectedItems={selectedTrendItems} onToggleItem={toggleTrendItem} collapsed={portfolioTrendCollapsed} onToggleCollapsed={() => setPortfolioTrendCollapsed(current => !current)} pickerLabel="그래프에 표시할 계좌" />
