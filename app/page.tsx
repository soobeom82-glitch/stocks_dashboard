...
function PerformancePanel({ title, period, onPeriodChange, items, aggregateSeries, selectedItems, onToggleItem, collapsed, onToggleCollapsed, pickerLabel, pickerMode = "side", pickerColumnLabel = "계좌", valuationDailyRate, itemDailyRates }: { title: string; period: ReportPeriod; onPeriodChange: (period: ReportPeriod) => void; items: TrendSeries[]; aggregateSeries: TrendSeries; selectedItems: string[]; onToggleItem: (id: string) => void; collapsed: boolean; onToggleCollapsed: () => void; pickerLabel: string; pickerMode?: "side" | "below"; pickerColumnLabel?: string; valuationDailyRate?: number | null; itemDailyRates?: Map<string, number | null> }) {
  ...
  const accountPicker = pickerMode === "below" && <div className="trend-account-picker" aria-label={pickerLabel}><div className="trend-account-picker-heading"><span>{pickerColumnLabel}</span><span>평가금액</span><span>수익률</span><span>평가손익</span><span>비중</span></div>{items.map(item => { const { rate, profit } = itemPerformance(item); const amount = item.snapshots.at(-1)?.total ?? 0; const tone = rate === null ? "" : rate >= 0 ? "positive" : "negative"; return <button key={item.id} className={selectedItems.includes(item.id) ? "selected" : ""} onClick={() => onToggleItem(item.id)} aria-pressed={selectedItems.includes(item.id)}><span className="trend-account-name"><i style={{ background: item.color }}>{item.iconLabel}</i><span><b>{item.name}</b>{item.subtitle && <small>{item.subtitle}</small>}</span></span><strong className="valuation-amount">{won.format(amount)}<PriorCloseRate rate={itemDailyRates?.get(item.id)} /></strong><strong className={tone}>{rate === null ? "-" : percent(rate)}</strong><strong className={tone}>{profit === null ? "-" : <>{profit >= 0 ? "+" : ""}{won.format(profit)}</>}</strong><span>{pickerTotal > 0 ? (amount / pickerTotal * 100).toFixed(1) : "0.0"}%</span></button>; })}</div>;
  return <section className="content-grid trend-content-grid"><article className="panel performance-panel"><header className="trend-panel-header"><div className="trend-heading"><div><h2>{title}</h2><p>{aggregateMode ? `전체 ${items.length}개 항목 합산` : `선택 항목 ${visibleItems.length}개 비교`}</p></div>{headline && <div className="trend-headline-metrics"><div><span>통합 수익률</span><strong className={headline.rate !== null && headline.rate < 0 ? "negative" : "positive"}>{headline.rate === null ? "-" : percent(headline.rate)}</strong></div><div><span>통합 평가금액</span><strong className="valuation-amount">{won.format(headlineAmount)}<PriorCloseRate rate={valuationDailyRate} /></strong></div><div><span>평가손익</span><strong className={headline.profit !== null && headline.profit < 0 ? "negative" : "positive"}>{headline.profit === null ? "-" : <>{headline.profit >= 0 ? "+" : ""}{won.format(headline.profit)}</>}</strong></div></div>}</div></header><div className="trend-panel-body"><div className={`trend-workspace ${pickerMode === "below" ? "without-selection" : ""}`}><div className="trend-chart-column"><div className="trend-series-list"><TrendChart series={chartSeries} stackSeries={aggregateMode ? items : visibleItems} showLegend={false} /></div><div className="periods" role="tablist">{reports.map(item => <button key={item} role="tab" aria-selected={period === item} className={period === item ? "selected" : ""} onClick={() => onPeriodChange(item)}>{item}</button>)}</div></div>{pickerMode === "side" && <aside className="trend-selection-panel" aria-label={pickerLabel}><div className="trend-checklist">{items.map(item => { const { rate, profit } = itemPerformance(item); const tone = rate === null ? "" : rate >= 0 ? "positive" : "negative"; return <button key={item.id} className={selectedItems.includes(item.id) ? "selected" : ""} onClick={() => onToggleItem(item.id)} aria-pressed={selectedItems.includes(item.id)}><i style={{ background: item.color }} /><span>{item.name}</span><strong className={tone}>{rate === null || profit === null ? "-" : <>{percent(rate)}<small>{profit >= 0 ? "+" : ""}{won.format(profit)}</small></>}</strong></button>; })}</div></aside>}</div>{pickerMode === "below" && <button className="trend-list-toggle" onClick={onToggleCollapsed} aria-expanded={!collapsed}>{collapsed ? `${pickerColumnLabel} 목록 펼치기` : `${pickerColumnLabel} 목록 접기`}<span aria-hidden="true">⌃</span></button>}{!collapsed && accountPicker}</div></article></section>;
}
...
function AccountDetails({ account, positions, updatedAt, exchangeRate, refresh, snapshots, selectedTrendItems, onToggleTrendItem }: { account: Account; positions: Holding[]; updatedAt: string; exchangeRate: number; refresh: () => Promise<void>; snapshots: Snapshot[]; selectedTrendItems: string[]; onToggleTrendItem: (id: string) => void }) {
  ...
  return <div className="account-expanded">
    ...
    {positions.length === 0 ? <div className="empty-holdings">등록된 {isCoin ? "코인 보유자산" : "보유 종목"}이 없습니다.</div> : groups.map(group => <div className="holding-group" key={group.assetClass}>{isIrp && <h4>{group.assetClass}</h4>}<div className="holding-table"><div><span>{isCoin ? "코인" : isIrp ? "상품" : "종목"}</span><span>보유 수량</span><span>매입금액</span><span>평가금액</span><span>평가손익</span><span>수익률</span></div>{group.positions.map(holding => {
      ...
      return <div key={`${holding.symbol}-${holding.name}`} className={selectedTrendItems.includes(trendId) ? "holding-trend-selected" : ""} role="button" tabIndex={0} onClick={event => { event.stopPropagation(); onToggleTrendItem(trendId); }} onKeyDown={event => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); event.stopPropagation(); onToggleTrendItem(trendId); } }} aria-pressed={selectedTrendItems.includes(trendId)}><b>{holding.name}</b><span>{isCoin ? `${holding.quantity.toLocaleString("ko-KR", { maximumFractionDigits: 8 })} ${holding.symbol}` : `${holding.quantity}${holding.unit ?? "주"}`}</span><span>{won.format(cost)}</span><span className="valuation-amount">{won.format(value)}<PriorCloseRate rate={dailyRate} /></span><strong className={profit >= 0 ? "positive" : "negative"}>{profit >= 0 ? "+" : ""}{won.format(profit)}</strong><strong className={rate >= 0 ? "positive" : "negative"}>{percent(rate)}</strong></div>;
    })}</div></div>)}
  </div>;
}
...
export default function Home() {
  ...
  const [assetTrendCollapsed, setAssetTrendCollapsed] = useState(true);
  ...
  const dailyValuations = useMemo(() => {
    ...
    sources.forEach(({ positions, exchangeRate }) => positions.forEach(holding => {
      ...
      holdingRates.set(holdingSnapshotKey(holding.accountId, holding), hasComparison && previous > 0 ? (current / previous - 1) * 100 : null);
    ...
    return { totalRate: hasTotalComparison && totalPrevious > 0 ? (total / totalPrevious - 1) * 100 : null, accountRates, assetRates, assetAccountRates, holdingRates };
  }, [portfolioAccounts, total, holdings, usdHoldings, usdKrwRate, fundHoldings, coinHoldings, pensionHoldings, isaHoldings, irpHoldings]);
  ...
  const currentHoldingAmounts = useMemo(() => {
  ...
  const domesticTopThree = useMemo(() => {
    const grouped = new Map<string, { name: string; symbol: string; value: number; previous: number; hasComparison: boolean; cost: number; quantity: number; accountNames: Set<string> }>();
    assetDetailsByType["국내 주식"].holdings.forEach(({ account, holding, value }) => {
      const key = holding.symbol || holding.name;
      const current = grouped.get(key) ?? { name: holding.name, symbol: holding.symbol, value: 0, previous: 0, hasComparison: false, cost: 0, quantity: 0, accountNames: new Set<string>() };
      current.value += value;
      const hasComparison = typeof holding.previousClose === "number" && holding.previousClose > 0;
      current.previous += hasComparison ? value * holding.previousClose! / holding.fallbackPrice : value;
      current.hasComparison ||= hasComparison;
