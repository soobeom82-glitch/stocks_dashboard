...
export default function Home() {
  const [accounts, setAccounts] = useState(initialAccounts); const [period, setPeriod] = useState(5); const [fileName, setFileName] = useState(""); const [imageData, setImageData] = useState(""); const [importOpen, setImportOpen] = useState(false); const [isAnalyzing, setIsAnalyzing] = useState(false); const [notice, setNotice] = useState(""); const [holdings, setHoldings] = useState<Holding[]>([]); const [quoteUpdatedAt, setQuoteUpdatedAt] = useState(""); const [manageOpen, setManageOpen] = useState(false); const [newType, setNewType] = useState("국내 주식"); const [newName, setNewName] = useState(""); const [newBroker, setNewBroker] = useState(""); const [importAccountId, setImportAccountId] = useState(""); const [imports, setImports] = useState<ScreenshotImport[]>([]); const [hydrated, setHydrated] = useState(false); const syncErrorShown = useRef(false);
  ...
  useEffect(() => {
    ...
    const loadPortfolio = async () => {
      try {
        ...
        const data = await response.json() as { hasData?: boolean; state?: { accounts?: Account[]; imports?: ScreenshotImport[]; holdings?: Holding[] } };
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      void fetch("/api/portfolio", {
        ...
        body: JSON.stringify({ accounts, imports, holdings }),
      }).then(response => {
  ...
  }, [accounts, imports, holdings, hydrated]);
  const chooseScreenshot = (event: ChangeEvent<HTMLInputElement>) => { const picked = event.target.files?.[0]; if (!picked) return; const reader = new FileReader(); reader.onload = () => { setFileName(picked.name); setImageData(String(reader.result)); setImportAccountId(""); setImportOpen(true); setNotice("스크린샷을 불러왔습니다. 반영할 계좌를 선택해 주세요."); }; reader.readAsDataURL(picked); };
  const importExample = async () => { const target = accounts.find(account => String(account.id) === importAccountId); if (!target || !imageData) { setNotice("스크린샷을 반영할 계좌를 선택해 주세요."); return; } setIsAnalyzing(true); try { const response = await fetch("/api/extract", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ image: imageData, accountType: target.type }) }); const data = await response.json() as { holdings?: Array<{ name: string; symbol: string | null; quantity: number | null; average_price: number | null }>; error?: string }; if (!response.ok || !data.holdings) throw new Error(data.error ?? "분석 실패"); const extracted = data.holdings.filter(item => item.quantity && item.average_price); if (target.type === "국내 주식" && extracted.length > 0) { const next = extracted.map(item => ({ name: item.name, symbol: item.symbol?.match(/^\d{6}$/) ? `${item.symbol}.KS` : (item.symbol ?? item.name), quantity: item.quantity!, averagePrice: item.average_price!, fallbackPrice: item.average_price! })); setHoldings(next); const amount = next.reduce((sum, item) => sum + item.quantity * item.averagePrice, 0); setAccounts(current => current.map(account => account.id === target.id ? { ...account, amount, returnRate: 0 } : account)); } setImports(current => [...current, { id: Date.now(), accountId: target.id, fileName, createdAt: new Date().toLocaleString("ko-KR"), status: "검토 필요", summary: `${extracted.length}개 종목 추출` }]); setImportOpen(false); setNotice(`“${target.name}”에서 ${extracted.length}개 보유 종목을 추출했습니다. 현재가를 반영해 검토해 주세요.`); } catch (error) { setNotice(error instanceof Error ? error.message : "이미지 분석에 실패했습니다."); } finally { setIsAnalyzing(false); } };
  const resetAccount = (account: Account) => {
    ...
    if (account.id === 2) setHoldings([]); setImports(current => current.filter(item => item.accountId !== account.id));
  ...
  return <main>
    ...
    <section className="hero"><div><p className="eyebrow">ALL ACCOUNTS · KRW</p><h1>내 자산, 한눈에.</h1><p className="hero-copy">증권사별 계좌와 연금·펀드·코인을 한곳에 모아 성과를 확인하세요.</p></div><label className="upload-button"><input aria-label="MTS 스크린샷 업로드" type="file" accept="image/*" onChange={chooseScreenshot}/><span>＋</span> MTS 스크린샷 가져오기</label></section>
    ...
    <section className="accounts-section"><div className="panel-head"><div><p className="eyebrow">ACCOUNTS</p><h2>계좌별 자산</h2></div><button className="text-button" onClick={() => setManageOpen(true)}>계좌 관리 →</button></div><div className="account-table"><div className="table-heading"><span>계좌</span><span>자산 유형</span><span>평가금액</span><span>수익률</span><span>비중</span><span/></div>{accounts.map(account => { const accountImports = imports.filter(item => item.accountId === account.id); return <div className="account-row" key={account.id}><div><span className={`account-icon ${account.color}`}>{account.type.slice(0, 1)}</span><span><b>{account.name}</b><small>{account.broker}{accountImports.length > 0 && ` · 스크린샷 ${accountImports.length}건`}</small></span></div><span className="type-chip">{account.type}</span><strong>{won.format(account.amount)}</strong><strong className={account.returnRate >= 0 ? "positive" : "negative"}>{percent(account.returnRate)}</strong><div className="weight"><i><em style={{ width: `${total > 0 ? account.amount / total * 100 : 0}%` }}/></i><span>{total > 0 ? (account.amount / total * 100).toFixed(1) : "0.0"}%</span></div><span className="row-actions"><button className="reset-button" onClick={() => resetAccount(account)}>초기화</button><button className="delete-button" onClick={() => deleteAccount(account)}>삭제</button></span></div>; })}</div><button className="add-account-inline" onClick={() => setManageOpen(true)}>＋ 계좌 추가</button></section>
    {imports.length > 0 && <section className="import-history"><div className="panel-head"><div><p className="eyebrow">SCREENSHOT IMPORTS</p><h2>계좌별 스크린샷 가져오기</h2></div></div>{imports.slice().reverse().map(item => { const target = accounts.find(account => account.id === item.accountId); return <div key={item.id}><span><b>{target?.name ?? "삭제된 계좌"}</b><small>{item.fileName} · {item.createdAt}</small></span><span className="import-status">{item.status}</span></div>; })}</section>}
    ...
    {importOpen && <div className="modal-backdrop"><section className="import-modal" role="dialog" aria-modal="true" aria-label="스크린샷 데이터 검토"><button className="modal-close" disabled={isAnalyzing} onClick={() => setImportOpen(false)}>×</button><p className="eyebrow">SCREENSHOT IMPORT</p><h2>반영할 계좌를 선택하세요</h2><p className="file-name">{fileName}</p><label className="import-account-select">대상 계좌<select disabled={isAnalyzing} value={importAccountId} onChange={event => setImportAccountId(event.target.value)}><option value="">계좌를 선택하세요</option>{accounts.map(account => <option key={account.id} value={account.id}>{account.name} · {account.type} · {account.broker}</option>)}</select></label><div className="review-box"><div><span>분석 기준</span><b>보유 수량 · 평단가</b></div><div><span>분석 상태</span><b>{isAnalyzing ? "이미지를 분석하는 중" : "분석 시작 전"}</b></div></div><p className="helper">선택한 계좌에만 이 스크린샷의 결과가 연결됩니다.</p><div className="modal-actions"><button className="cancel" disabled={isAnalyzing} onClick={() => setImportOpen(false)}>취소</button><button className="confirm" disabled={isAnalyzing} onClick={() => void importExample()}>{isAnalyzing ? "분석 중…" : "분석하고 반영"}</button></div></section></div>}
