...
export default function Home() {
  ...
  const todaySummary = useMemo(() => {
    ...
    const falling = dailyContributions.find(item => item.amountChange < 0);
    const parts = [`전체 자산 ${percent(dailyValuations.totalRate)}.`];
    if (rising) parts.push(`${rising.name} 상승이 주도했고`);
    if (falling) parts.push(`${falling.name} 하락이 일부 상쇄했습니다.`);
    return parts.join(" ");
  }, [dailyContributions, dailyValuations.totalRate]);
  ...
  return <main>
    ...
    <section className="today-summary"><span>오늘의 요약</span><p>{todaySummary}</p></section>
