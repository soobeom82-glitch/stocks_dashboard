...
function AccountDetails({ account, holdings, coinHoldings, pensionHoldings, quoteUpdatedAt, coinQuoteUpdatedAt, refreshPrices, refreshCoinPrices }: { account: Account; holdings: Holding[]; coinHoldings: CoinHolding[]; pensionHoldings: PensionHolding[]; quoteUpdatedAt: string; coinQuoteUpdatedAt: string; refreshPrices: () => Promise<void>; refreshCoinPrices: () => Promise<void> }) {
  ...
  if (!isDomestic && !isCoin && !isPension) return <div className="account-expanded empty-account-detail">등록된 보유자산이 없습니다.</div>;
  return <div className="account-expanded">
    <div className="detail-head"><div><p className="eyebrow">{isDomestic ? "DOMESTIC HOLDINGS" : isCoin ? "CRYPTO HOLDINGS" : "PENSION HOLDINGS"}</p><h3>{isDomestic ? "국내 주식 · 현재가 기준" : isCoin ? "코인 · 업비트 현재가 기준" : "연금저축 · 마지막 등록 현재가 기준"}</h3></div>{!isPension && <button className="text-button" onClick={event => { event.stopPropagation(); void refresh(); }}>현재가 새로고침 {updatedAt && `· ${updatedAt}`}</button>}</div>
    <p className="holdings-note">{isDomestic ? "등록된 보유 수량과 평단가를 기준으로 현재가 손익과 수익률을 계산합니다." : isCoin ? "업비트 KRW 마켓 현재가를 6시간마다 갱신하며, 버튼으로 즉시 다시 조회할 수 있습니다." : "등록된 보유 수량과 평단가·현재가를 기준으로 손익과 수익률을 계산합니다."}</p>
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const loadPortfolio = async () => {
      try {
        ...
        const data = await response.json() as { hasData?: boolean; state?: { accounts?: Account[]; imports?: ScreenshotImport[]; holdings?: Holding[]; coinHoldings?: CoinHolding[]; pensionHoldings?: PensionHolding[] } };
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      void fetch("/api/portfolio", {
        ...
        body: JSON.stringify({ accounts, imports, holdings, coinHoldings, pensionHoldings }),
      }).then(response => {
  ...
  }, [accounts, imports, holdings, coinHoldings, pensionHoldings, hydrated]);
