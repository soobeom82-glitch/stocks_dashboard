...
export default function Home() {
  ...
  return <main>
    ...
    <section className="panel holdings-top-panel"><div className="panel-head"><div><h2>전체 보유자산 Top 10</h2><p className="account-hint">동일 티커·시장 보유분을 상태와 관계없이 합산합니다. 종목을 누르면 계좌별 출처를 볼 수 있습니다.</p></div><span className="domestic-total">평가금액 기준</span></div><div className="holdings-top-table"><div className="holdings-top-heading"><span>종목</span><span>유형</span><span>평가금액</span><span>전체 비중</span><span>수익률</span></div>{aggregatedHoldings.slice(0, 10).map((item, index) => <div className="holding-top-item" key={item.id}><button className={`holdings-top-row ${expandedTopHoldingId === item.id ? "expanded" : ""}`} onClick={() => setExpandedTopHoldingId(current => current === item.id ? null : item.id)} aria-expanded={expandedTopHoldingId === item.id}><span className="holding-top-name"><i>{index + 1}</i><span><b>{item.name}</b>{item.ticker && <small>{item.ticker}</small>}</span></span><span className={`asset-type-chip ${assetTypeMeta[item.assetType].color}`}>{item.assetType}</span><strong>{won.format(item.evaluationAmount)}</strong><strong>{item.portfolioWeight.toFixed(1)}%</strong><strong className={item.returnRate === null || item.returnRate >= 0 ? "positive" : "negative"}>{item.returnRate === null ? "-" : percent(item.returnRate)}</strong></button>{expandedTopHoldingId === item.id && <div className="holding-source-detail"><div className="holding-source-heading"><span>계좌 · 보유 상태</span><span>수량</span><span>평가금액</span><span>종목 내 비중</span></div>{item.lots.sort((left, right) => right.evaluationAmount - left.evaluationAmount).map((lot, lotIndex) => <div className="holding-source-row" key={`${lot.accountId}-${lot.holdingStatus}-${lotIndex}`}><span><b>{accountLabel(lot.accountName)}</b><small>{lot.holdingStatus}</small></span><strong>{lot.quantity > 0 ? lot.quantity.toLocaleString("ko-KR", { maximumFractionDigits: 6 }) : "-"}</strong><strong>{won.format(lot.evaluationAmount)}</strong><strong>{item.evaluationAmount > 0 ? (lot.evaluationAmount / item.evaluationAmount * 100).toFixed(1) : "0.0"}%</strong></div>)}</div>}</div>)}</div></section>
