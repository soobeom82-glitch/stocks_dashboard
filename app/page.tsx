...
function PerformancePanel({ title, period, onPeriodChange, items, aggregateSeries, selectedItems, onToggleItem, collapsed, onToggleCollapsed, pickerLabel, pickerMode = "side", pickerColumnLabel = "계좌" }: { title: string; period: ReportPeriod; onPeriodChange: (period: ReportPeriod) => void; items: TrendSeries[]; aggregateSeries: TrendSeries; selectedItems: string[]; onToggleItem: (id: string) => void; collapsed: boolean; onToggleCollapsed: () => void; pickerLabel: string; pickerMode?: "side" | "below"; pickerColumnLabel?: string }) {
  ...
  return <section className="content-grid trend-content-grid"><article className={`panel performance-panel ${collapsed ? "collapsed" : ""}`}><header className="trend-panel-header"><div className="trend-heading"><div><h2>{title}</h2><p>{collapsed ? "접힌 상태입니다" : aggregateMode ? `전체 ${items.length}개 항목 합산` : `선택 항목 ${visibleItems.length}개 비교`}</p></div>{!collapsed && headline && <div className="trend-headline-metrics"><div><span>통합 수익률</span><strong className={headline.rate !== null && headline.rate < 0 ? "negative" : "positive"}>{headline.rate === null ? "-" : percent(headline.rate)}</strong></div><div><span>평가손익</span><strong className={headline.profit !== null && headline.profit < 0 ? "negative" : "positive"}>{headline.profit === null ? "-" : <>{headline.profit >= 0 ? "+" : ""}{won.format(headline.profit)}</>}</strong></div></div>}</div><button className="trend-collapse-button" onClick={onToggleCollapsed} aria-expanded={!collapsed}>{collapsed ? "펼치기" : "접기"}<span aria-hidden="true">⌃</span></button></header>{!collapsed && <div className="trend-panel-body"><div className={`trend-workspace ${pickerMode === "below" ? "without-selection" : ""}`}><div className="trend-chart-column"><div className="trend-series-list"><TrendChart series={chartSeries} showLegend={false} /></div><div className="periods" role="tablist">{reports.map(item => <button key={item} role="tab" aria-selected={period === item} className={period === item ? "selected" : ""} onClick={() => onPeriodChange(item)}>{item}</button>)}</div></div>{pickerMode === "side" && <aside className="trend-selection-panel" aria-label={pickerLabel}><div className="trend-checklist">{items.map(item => { const { rate, profit } = itemPerformance(item); const tone = rate === null ? "" : rate >= 0 ? "positive" : "negative"; return <button key={item.id} className={selectedItems.includes(item.id) ? "selected" : ""} onClick={() => onToggleItem(item.id)} aria-pressed={selectedItems.includes(item.id)}><i style={{ background: item.color }} /><span>{item.name}</span><strong className={tone}>{rate === null || profit === null ? "-" : <>{percent(rate)}<small>{profit >= 0 ? "+" : ""}{won.format(profit)}</small></>}</strong></button>; })}</div></aside>}</div>{accountPicker}</div>}</article></section>;
}
...
export default function Home() {
  ...
  const [portfolioTrendCollapsed, setPortfolioTrendCollapsed] = useState(false);
  const [assetTrendCollapsed, setAssetTrendCollapsed] = useState(false);
