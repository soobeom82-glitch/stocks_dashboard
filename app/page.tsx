...
export default function Home() {
  ...
  return <main>
    ...
    <section className="panel domestic-top-panel"><div className="panel-head"><div><p className="eyebrow">DOMESTIC EQUITY TOP 3</p><h2>국내 주식 보유금액 Top 3</h2></div><span className="domestic-total">평가금액 기준</span></div><p className="account-hint">여러 계좌에 겹쳐 보유한 동일 종목은 합산합니다.</p>{domesticTopThree.items.length ? <div className="domestic-top-chart"><div className="domestic-top-heading"><span>종목</span><span>평가금액</span><span>수익금 · 수익률</span></div>{domesticTopThree.items.map((item, index) => { const profit = item.value - item.cost; const rate = item.cost > 0 ? (item.value / item.cost - 1) * 100 : 0; return <div className="domestic-top-row" key={item.symbol || item.name}><span className="domestic-rank">{index + 1}</span><div className="domestic-name"><b>{item.name}</b><small>{item.symbol} · {item.quantity.toLocaleString("ko-KR")}주 · {[...item.accountNames].join(", ")}</small></div><div className="domestic-value"><strong>{won.format(item.value)}</strong><small>현재 평가금액</small></div><div className="domestic-return"><strong className={profit >= 0 ? "positive" : "negative"}>{profit >= 0 ? "+" : ""}{won.format(profit)}</strong><span className={rate >= 0 ? "positive" : "negative"}>{percent(rate)}</span></div></div>; })}</div> : <div className="empty-holdings">국내 주식 보유 종목을 등록하면 상위 3개가 표시됩니다.</div>}</section>
