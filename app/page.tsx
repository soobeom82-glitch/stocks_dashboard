...
function PerformancePanel({ title, period, onPeriodChange, items, aggregateSeries, selectedItems, onToggleItem, collapsed, onToggleCollapsed, pickerLabel }: { title: string; period: ReportPeriod; onPeriodChange: (period: ReportPeriod) => void; items: TrendSeries[]; aggregateSeries: TrendSeries; selectedItems: string[]; onToggleItem: (id: string) => void; collapsed: boolean; onToggleCollapsed: () => void; pickerLabel: string }) {
  ...
  const chartSeries = allItemsSelected ? [aggregateSeries] : visibleItems;
  return <section className="content-grid trend-content-grid"><article className={`panel performance-panel ${collapsed ? "collapsed" : ""}`}><header className="trend-panel-header"><div><h2>{title}</h2><p>{collapsed ? "접힌 상태입니다" : allItemsSelected ? `전체 ${items.length}개 항목 합산 · 수익률과 평가손익` : `선택 항목 ${visibleItems.length}개 · 항목별 수익률과 평가손익`}</p></div><button className="trend-collapse-button" onClick={onToggleCollapsed} aria-expanded={!collapsed}>{collapsed ? "펼치기" : "접기"}<span aria-hidden="true">⌃</span></button></header>{!collapsed && <div className="trend-panel-body"><div className="trend-workspace"><div className="trend-chart-column"><div className="trend-series-list">{chartSeries.length ? chartSeries.map(item => <TrendChart key={item.id} series={[item]} />) : <div className="empty-holdings">그래프로 볼 항목을 하나 이상 선택해 주세요.</div>}</div><div className="periods" role="tablist">{reports.map(item => <button key={item} role="tab" aria-selected={period === item} className={period === item ? "selected" : ""} onClick={() => onPeriodChange(item)}>{item}</button>)}</div></div><aside className="trend-selection-panel" aria-label={pickerLabel}><div className="trend-checklist">{items.map(item => <button key={item.id} className={selectedItems.includes(item.id) ? "selected" : ""} onClick={() => onToggleItem(item.id)} aria-pressed={selectedItems.includes(item.id)}><i style={{ background: item.color }} /><span>{item.name}</span></button>)}</div></aside></div></div>}</article></section>;
}
