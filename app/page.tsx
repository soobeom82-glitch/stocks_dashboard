...
type Snapshot = { date: string; total: number; accountAmounts?: Record<string, number>; assetAmounts?: Partial<Record<AssetType, number>> };
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    setSnapshots(current => {
      ...
      if (existing) return existing.accountAmounts && existing.assetAmounts ? current : current.map(snapshot => snapshot.date === date ? { ...snapshot, accountAmounts, assetAmounts } : snapshot);
      return [...current, { date, total, accountAmounts, assetAmounts }].sort((a, b) => a.date.localeCompare(b.date));
    });
  }, [accounts, assetAllocationByType, hydrated, total]);
  useEffect(() => {
    if (!hydrated) return;
    const date = todayKst();
