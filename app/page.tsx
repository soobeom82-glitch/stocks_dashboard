...
export default function Home() {
  ...
  const activeTargets = activePortfolio ? targetAllocations[activePortfolio.id] ?? {} : {};
  ...
  return <main>
    ...
    <section className="panel holdings-top-panel"><div className="panel-head"><div><h2>전체 보유자산 Top 10</h2><p className="account-hint">동일 티커·시장 보유분을 계좌별로 합산해 평가금액 순으로 표시합니다.</p></div><span className="domestic-total">평가금액 기준</span></div><div className="holdings-top-table"><div className="holdings-top-heading"><span>종목</span><span>유형</span><span>평가금액</span><span>전체 비중</span><span>수익률</span></div>{aggregatedHoldings.slice(0, 10).map((item, index) => <div className="holdings-top-row" key={item.id}><span className="holding-top-name"><i>{index + 1}</i><span><b>{item.name}</b>{item.ticker && <small>{item.ticker}</small>}</span></span><span className={`asset-type-chip ${assetTypeMeta[item.assetType].color}`}>{item.assetType}</span><strong>{won.format(item.evaluationAmount)}</strong><strong>{item.portfolioWeight.toFixed(1)}%</strong><strong className={item.returnRate === null || item.returnRate >= 0 ? "positive" : "negative"}>{item.returnRate === null ? "-" : percent(item.returnRate)}</strong></div>)}</div></section>
