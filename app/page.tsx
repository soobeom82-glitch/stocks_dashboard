...
function TrendChart({ series, showLegend = true }: { series: TrendSeries[]; showLegend?: boolean }) {
  ...
  const profitStacks = drawableSeries[0].snapshots.map((_, index) => drawableSeries.reduce((totals, item) => { const profit = item.profits[index] ?? 0; return profit >= 0 ? { ...totals, positive: totals.positive + profit } : { ...totals, negative: totals.negative + profit }; }, { positive: 0, negative: 0 }));
  const axisProfit = Math.max(...profitStacks.flatMap(stack => [stack.positive, Math.abs(stack.negative)]), 1) * 1.15;
  ...
  const barWidth = Math.min(8, 54 / profitStacks.length);
  return <>{showLegend && <TrendLegend series={series} />}<div className="trend-chart"><div className="trend-axis trend-rate-axis" aria-label="수익률 축"><span>{rateLabel(axisRate)}</span><span>0.00%</span><span>{rateLabel(-axisRate)}</span></div><div className="trend-plot"><svg viewBox="0 0 100 100" preserveAspectRatio="none" role="img" aria-label="항목별 수익률 선과 항목별 누적 평가손익 스택 막대 추이"><g className="trend-profit-bars">{profitStacks.map((_, index) => { const x = profitStacks.length === 1 ? 50 : index / (profitStacks.length - 1) * 100; let positive = 0; let negative = 0; return drawableSeries.map(item => { const profit = item.profits[index] ?? 0; if (!profit) return null; const start = profit >= 0 ? positive : negative; if (profit >= 0) positive += profit; else negative += profit; const end = profit >= 0 ? positive : negative; const startY = 50 - start / axisProfit * 40; const endY = 50 - end / axisProfit * 40; return <rect key={`${item.id}-${index}`} x={x - barWidth / 2} y={Math.min(startY, endY)} width={barWidth} height={Math.abs(startY - endY)} fill={item.color} />; }); })}</g>{drawableSeries.map(item => { const points = item.returns.map((rate, index) => `${item.returns.length === 1 ? 50 : index / (item.returns.length - 1) * 100},${50 - rate / axisRate * 40}`).join(" "); return <polyline key={item.id} points={points} fill="none" stroke={item.color} strokeWidth="2.5" vectorEffect="non-scaling-stroke" />; })}</svg></div><div className="trend-axis trend-profit-axis" aria-label="누적 평가손익 축"><span>{profitLabel(axisProfit)}</span><span>0</span><span>{profitLabel(-axisProfit)}</span></div><div className="trend-labels"><span>{firstDate.slice(5).replace("-", ".")}</span><span>{lastDate.slice(5).replace("-", ".")}</span></div></div></>;
}

function PerformancePanel({ title, period, onPeriodChange, items, aggregateSeries, selectedItems, onToggleItem, collapsed, onToggleCollapsed, pickerLabel, pickerMode = "side", pickerColumnLabel = "계좌" }: { title: string; period: ReportPeriod; onPeriodChange: (period: ReportPeriod) => void; items: TrendSeries[]; aggregateSeries: TrendSeries; selectedItems: string[]; onToggleItem: (id: string) => void; collapsed: boolean; onToggleCollapsed: () => void; pickerLabel: string; pickerMode?: "side" | "below"; pickerColumnLabel?: string }) {
...
function AccountDetails({ account, positions, updatedAt, exchangeRate, refresh, snapshots, selectedTrendItems, onToggleTrendItem }: { account: Account; positions: Holding[]; updatedAt: string; exchangeRate: number; refresh: () => Promise<void>; snapshots: Snapshot[]; selectedTrendItems: string[]; onToggleTrendItem: (id: string) => void }) {
  ...
  return <div className="account-expanded">
    ...
    {positions.length === 0 ? <div className="empty-holdings">등록된 {isCoin ? "코인 보유자산" : "보유 종목"}이 없습니다.</div> : groups.map(group => <div className="holding-group" key={group.assetClass}>{isIrp && <h4>{group.assetClass}</h4>}<div className="holding-table"><div><span>{isCoin ? "코인" : isIrp ? "상품" : "종목"}</span><span>보유 수량</span><span>매입금액</span><span>평가금액</span><span>이전 종가 대비</span><span>평가손익</span><span>수익률</span></div>{group.positions.map(holding => {
      ...
      return <div key={`${holding.symbol}-${holding.name}`} className={selectedTrendItems.includes(trendId) ? "holding-trend-selected" : ""} role="button" tabIndex={0} onClick={event => { event.stopPropagation(); onToggleTrendItem(trendId); }} onKeyDown={event => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); event.stopPropagation(); onToggleTrendItem(trendId); } }} aria-pressed={selectedTrendItems.includes(trendId)}><b>{holding.name}<small>{isKrwStock ? holding.symbol.replace(".KS", "") : holding.symbol}{isUsd ? ` · $${holding.fallbackPrice.toFixed(2)}` : ""}{isIrp && holding.marketPrice ? ` · 현재가 ${won.format(holding.marketPrice)}` : ""}</small></b><span>{isCoin ? `${holding.quantity.toLocaleString("ko-KR", { maximumFractionDigits: 8 })} ${holding.symbol}` : `${holding.quantity}${holding.unit ?? "주"}`}</span><span>{won.format(cost)}</span><span>{won.format(value)}</span><strong className={dailyRate === null ? "muted-cell" : dailyRate >= 0 ? "positive" : "negative"}>{dailyRate === null ? "—" : percent(dailyRate)}</strong><strong className={profit >= 0 ? "positive" : "negative"}>{profit >= 0 ? "+" : ""}{won.format(profit)}</strong><strong className={rate >= 0 ? "positive" : "negative"}>{percent(rate)}</strong></div>;
    })}</div></div>)}
  </div>;
}
