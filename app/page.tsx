...
type ScreenshotImport = { id: number; accountId: number; fileName: string; createdAt: string; status: "추출 대기" | "검토 필요" };
...
export default function Home() {
  ...
  useEffect(() => {
    try {
      ...
      if (saved) {
        const parsed = JSON.parse(saved) as { accounts?: Account[]; imports?: ScreenshotImport[]; holdings?: Holding[] };
    ...
    } catch { setNotice("저장된 계좌 데이터를 불러오지 못했습니다."); }
    setHydrated(true);
  }, []);
  useEffect(() => {
    if (!hydrated) return;
    localStorage.setItem(STORAGE_KEY, JSON.stringify({ accounts, imports, holdings }));
  }, [accounts, imports, holdings, hydrated]);
  const chooseScreenshot = (event: ChangeEvent<HTMLInputElement>) => { const picked = event.target.files?.[0]; if (!picked) return; setFileName(picked.name); setImportAccountId(""); setImportOpen(true); setNotice("스크린샷을 불러왔습니다. 반영할 계좌를 선택해 주세요."); };
  const importExample = () => { const target = accounts.find(account => String(account.id) === importAccountId); if (!target) { setNotice("스크린샷을 반영할 계좌를 선택해 주세요."); return; } setImports(current => [...current, { id: Date.now(), accountId: target.id, fileName, createdAt: new Date().toLocaleString("ko-KR"), status: "추출 대기" }]); setImportOpen(false); setNotice(`“${target.name}” 계좌에 스크린샷을 등록했습니다. 계좌별 가져오기 목록에서 결과를 확인할 수 있습니다.`); };
  ...
  return <main>
    ...
    {importOpen && <div className="modal-backdrop"><section className="import-modal" role="dialog" aria-modal="true" aria-label="스크린샷 데이터 검토"><button className="modal-close" onClick={() => setImportOpen(false)}>×</button><p className="eyebrow">SCREENSHOT IMPORT</p><h2>반영할 계좌를 선택하세요</h2><p className="file-name">{fileName}</p><label className="import-account-select">대상 계좌<select value={importAccountId} onChange={event => setImportAccountId(event.target.value)}><option value="">계좌를 선택하세요</option>{accounts.map(account => <option key={account.id} value={account.id}>{account.name} · {account.type} · {account.broker}</option>)}</select></label><div className="review-box"><div><span>인식 상태</span><b>추출 규칙 연결 대기</b></div><div><span>반영 기준</span><b>보유 수량 · 평단가</b></div></div><p className="helper">선택한 계좌에만 이 스크린샷의 인식 결과가 연결됩니다. 예시 종목이나 금액은 자동으로 넣지 않습니다.</p><div className="modal-actions"><button className="cancel" onClick={() => setImportOpen(false)}>취소</button><button className="confirm" onClick={importExample}>선택 계좌에 등록</button></div></section></div>}
