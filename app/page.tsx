...
function AccountDetails({ account, holdings, coinHoldings, quoteUpdatedAt, coinQuoteUpdatedAt, refreshPrices, refreshCoinPrices }: { account: Account; holdings: Holding[]; coinHoldings: CoinHolding[]; quoteUpdatedAt: string; coinQuoteUpdatedAt: string; refreshPrices: () => Promise<void>; refreshCoinPrices: () => Promise<void> }) {
  const isDomestic = account.type === "국내 주식";
  const isCoin = account.type === "코인";
  const positions = isDomestic ? holdings : isCoin ? coinHoldings : [];
  const updatedAt = isDomestic ? quoteUpdatedAt : coinQuoteUpdatedAt;
  const refresh = isDomestic ? refreshPrices : refreshCoinPrices;

  if (!isDomestic && !isCoin) return <div className="account-expanded empty-account-detail">등록된 보유자산이 없습니다.</div>;
  return <div className="account-expanded">
    <div className="detail-head"><div><p className="eyebrow">{isDomestic ? "DOMESTIC HOLDINGS" : "CRYPTO HOLDINGS"}</p><h3>{isDomestic ? "국내 주식 · 현재가 기준" : "코인 · 업비트 현재가 기준"}</h3></div><button className="text-button" onClick={event => { event.stopPropagation(); void refresh(); }}>현재가 새로고침 {updatedAt && `· ${updatedAt}`}</button></div>
    <p className="holdings-note">{isDomestic ? "등록된 보유 수량과 평단가를 기준으로 현재가 손익과 수익률을 계산합니다." : "업비트 KRW 마켓 현재가를 6시간마다 갱신하며, 버튼으로 즉시 다시 조회할 수 있습니다."}</p>
    {positions.length === 0 ? <div className="empty-holdings">등록된 {isDomestic ? "보유 종목" : "코인 보유자산"}이 없습니다.</div> : <div className="holding-table"><div><span>{isDomestic ? "종목" : "코인"}</span><span>보유 수량</span><span>평단가</span><span>현재가</span><span>평가손익</span><span>수익률</span></div>{positions.map(holding => { const profit = holding.quantity * (holding.fallbackPrice - holding.averagePrice); const rate = (holding.fallbackPrice / holding.averagePrice - 1) * 100; return <div key={holding.symbol}><b>{holding.name}<small>{isDomestic ? holding.symbol.replace(".KS", "") : holding.symbol}</small></b><span>{isDomestic ? `${holding.quantity}주` : `${holding.quantity.toLocaleString("ko-KR", { maximumFractionDigits: 8 })} ${holding.symbol}`}</span><span>{won.format(holding.averagePrice)}</span><span>{won.format(holding.fallbackPrice)}</span><strong className={profit >= 0 ? "positive" : "negative"}>{profit >= 0 ? "+" : ""}{won.format(profit)}</strong><strong className={rate >= 0 ? "positive" : "negative"}>{percent(rate)}</strong></div>; })}</div>}
  </div>;
}
