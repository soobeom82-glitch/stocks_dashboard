...
function AccountDetails({ account, holdings, coinHoldings, pensionHoldings, isaHoldings, irpHoldings, quoteUpdatedAt, coinQuoteUpdatedAt, isaQuoteUpdatedAt, pensionQuoteUpdatedAt, irpQuoteUpdatedAt, refreshPrices, refreshCoinPrices, refreshIsaPrices, refreshPensionPrices, refreshIrpPrices }: { account: Account; holdings: Holding[]; coinHoldings: CoinHolding[]; pensionHoldings: PensionHolding[]; isaHoldings: Holding[]; irpHoldings: Holding[]; quoteUpdatedAt: string; coinQuoteUpdatedAt: string; isaQuoteUpdatedAt: string; pensionQuoteUpdatedAt: string; irpQuoteUpdatedAt: string; refreshPrices: () => Promise<void>; refreshCoinPrices: () => Promise<void>; refreshIsaPrices: () => Promise<void>; refreshPensionPrices: () => Promise<void>; refreshIrpPrices: () => Promise<void> }) {
  ...
  const positions = isDomestic ? holdings.filter(holding => holding.accountId === account.id) : isCoin ? coinHoldings.filter(holding => holding.accountId === account.id) : isPension ? pensionHoldings.filter(holding => holding.accountId === account.id) : isIsa ? isaHoldings.filter(holding => holding.accountId === account.id) : isIrp ? irpHoldings.filter(holding => holding.accountId === account.id) : [];
  const updatedAt = isDomestic ? quoteUpdatedAt : isCoin ? coinQuoteUpdatedAt : isIsa ? isaQuoteUpdatedAt : isPension ? pensionQuoteUpdatedAt : irpQuoteUpdatedAt;
  const refresh = isDomestic ? refreshPrices : isCoin ? refreshCoinPrices : isIsa ? refreshIsaPrices : isPension ? refreshPensionPrices : refreshIrpPrices;

  const positionGroups = isIrp ? ["ETF·주식", "현금성·금융상품"].map(assetClass => ({ assetClass, positions: positions.filter(holding => holding.assetClass === assetClass) })).filter(group => group.positions.length > 0) : [{ assetClass: "보유자산", positions }];
  if (!isDomestic && !isCoin && !isPension && !isIsa && !isIrp) return <div className="account-expanded empty-account-detail">등록된 보유자산이 없습니다.</div>;
  return <div className="account-expanded">
    <div className="detail-head"><div><p className="eyebrow">{isDomestic ? "DOMESTIC HOLDINGS" : isCoin ? "CRYPTO HOLDINGS" : isPension ? "PENSION HOLDINGS" : isIsa ? "ISA HOLDINGS" : "IRP HOLDINGS"}</p><h3>{isDomestic ? "국내 주식 · 현재가 기준" : isCoin ? "코인 · 업비트 현재가 기준" : isPension ? "연금저축 · 현재가 기준" : isIsa ? "ISA · 현재가 기준" : "IRP · 투자상품 및 현금성 자산"}</h3></div><button className="text-button" onClick={event => { event.stopPropagation(); void refresh(); }}>현재가 새로고침 {updatedAt && `· ${updatedAt}`}</button></div>
    <p className="holdings-note">{isDomestic || isIsa || isPension ? "등록된 보유 수량과 평단가를 기준으로 현재가 손익과 수익률을 계산합니다." : isCoin ? "업비트 KRW 마켓 현재가를 6시간마다 갱신하며, 버튼으로 즉시 다시 조회할 수 있습니다." : "상장 ETF·주식은 현재가를 표시하고, 예금·보험·디폴트옵션은 마지막 등록 평가금액을 유지합니다."}</p>
    {positions.length === 0 ? <div className="empty-holdings">등록된 {isCoin ? "코인 보유자산" : "보유 종목"}이 없습니다.</div> : positionGroups.map(group => <div className="holding-group" key={group.assetClass}>{isIrp && <h4>{group.assetClass}</h4>}<div className="holding-table"><div><span>{isCoin ? "코인" : isIrp ? "상품" : "종목"}</span><span>보유 수량</span><span>매입금액</span><span>평가금액</span><span>평가손익</span><span>수익률</span></div>{group.positions.map(holding => { const profit = holding.quantity * (holding.fallbackPrice - holding.averagePrice); const rate = (holding.fallbackPrice / holding.averagePrice - 1) * 100; return <div key={`${holding.symbol}-${holding.name}`}><b>{holding.name}<small>{isDomestic || isIsa || isPension ? holding.symbol.replace(".KS", "") : holding.symbol}{isIrp && holding.marketPrice ? ` · 현재가 ${won.format(holding.marketPrice)}` : ""}</small></b><span>{isCoin ? `${holding.quantity.toLocaleString("ko-KR", { maximumFractionDigits: 8 })} ${holding.symbol}` : `${holding.quantity}${holding.unit ?? "주"}`}</span><span>{won.format(holding.quantity * holding.averagePrice)}</span><span>{won.format(holding.quantity * holding.fallbackPrice)}</span><strong className={profit >= 0 ? "positive" : "negative"}>{profit >= 0 ? "+" : ""}{won.format(profit)}</strong><strong className={rate >= 0 ? "positive" : "negative"}>{percent(rate)}</strong></div>; })}</div></div>)}
  </div>;
}

export default function Home() {
  const [accounts, setAccounts] = useState(initialAccounts); const [period, setPeriod] = useState(5); const [notice, setNotice] = useState(""); const [holdings, setHoldings] = useState<Holding[]>([]); const [coinHoldings, setCoinHoldings] = useState<CoinHolding[]>([]); const [pensionHoldings, setPensionHoldings] = useState<PensionHolding[]>([]); const [isaHoldings, setIsaHoldings] = useState<Holding[]>([]); const [irpHoldings, setIrpHoldings] = useState<Holding[]>([]); const [quoteUpdatedAt, setQuoteUpdatedAt] = useState(""); const [coinQuoteUpdatedAt, setCoinQuoteUpdatedAt] = useState(""); const [isaQuoteUpdatedAt, setIsaQuoteUpdatedAt] = useState(""); const [pensionQuoteUpdatedAt, setPensionQuoteUpdatedAt] = useState(""); const [irpQuoteUpdatedAt, setIrpQuoteUpdatedAt] = useState(""); const [expandedAccountId, setExpandedAccountId] = useState<number | null>(null); const [manageOpen, setManageOpen] = useState(false); const [newType, setNewType] = useState("국내 주식"); const [newName, setNewName] = useState(""); const [newBroker, setNewBroker] = useState(""); const [imports, setImports] = useState<ScreenshotImport[]>([]); const [hydrated, setHydrated] = useState(false); const syncErrorShown = useRef(false);
  ...
  const refreshIsaPrices = async () => {
    if (isaHoldings.length === 0) { setNotice("현재가를 반영할 ISA 보유 종목이 없습니다."); return; }
    try {
      const response = await fetch(`/api/quotes?symbols=${encodeURIComponent(isaHoldings.map(holding => holding.symbol).join(","))}`);
      const data = await response.json() as { quotes?: Record<string, number> };
      if (!data.quotes) throw new Error("No quotes");
      setIsaHoldings(current => {
        const next = current.map(holding => ({ ...holding, fallbackPrice: data.quotes?.[holding.symbol] ?? holding.fallbackPrice }));
        const value = next.reduce((sum, holding) => sum + holding.quantity * holding.fallbackPrice, 0);
        const cost = next.reduce((sum, holding) => sum + holding.quantity * holding.averagePrice, 0);
        setAccounts(accountState => accountState.map(account => account.id === 3 ? { ...account, amount: value, returnRate: cost > 0 ? (value / cost - 1) * 100 : 0 } : account));
        return next;
      });
      setIsaQuoteUpdatedAt(new Date().toLocaleTimeString("ko-KR", { hour: "2-digit", minute: "2-digit" }));
    } catch { setNotice("ISA 현재가를 불러오지 못했습니다. 마지막 확인 가격으로 계산합니다."); }
  };
  const refreshPensionPrices = async () => {
    const symbols = pensionHoldings.map(holding => holding.symbol).filter(symbol => symbol.endsWith(".KS"));
    if (symbols.length === 0) { setNotice("현재가를 반영할 연금저축 ETF가 없습니다."); return; }
    try {
      const response = await fetch(`/api/quotes?symbols=${encodeURIComponent(symbols.join(","))}`);
      const data = await response.json() as { quotes?: Record<string, number> };
      if (!data.quotes) throw new Error("No quotes");
      setPensionHoldings(current => {
        const next = current.map(holding => ({ ...holding, fallbackPrice: data.quotes?.[holding.symbol] ?? holding.fallbackPrice }));
        const value = next.reduce((sum, holding) => sum + holding.quantity * holding.fallbackPrice, 0);
        const cost = next.reduce((sum, holding) => sum + holding.quantity * holding.averagePrice, 0);
        setAccounts(accountState => accountState.map(account => account.id === 5 ? { ...account, amount: value, returnRate: cost > 0 ? (value / cost - 1) * 100 : 0 } : account));
        return next;
      });
      setPensionQuoteUpdatedAt(new Date().toLocaleTimeString("ko-KR", { hour: "2-digit", minute: "2-digit" }));
    } catch { setNotice("연금저축 현재가를 불러오지 못했습니다. 마지막 확인 가격으로 계산합니다."); }
  };
  const refreshIrpPrices = async () => {
    const symbols = irpHoldings.filter(holding => holding.assetClass === "ETF·주식" && holding.symbol.endsWith(".KS")).map(holding => holding.symbol);
    if (symbols.length === 0) { setNotice("현재가를 반영할 IRP ETF가 없습니다."); return; }
    try {
      const response = await fetch(`/api/quotes?symbols=${encodeURIComponent(symbols.join(","))}`);
      const data = await response.json() as { quotes?: Record<string, number> };
      if (!data.quotes) throw new Error("No quotes");
      setIrpHoldings(current => current.map(holding => ({ ...holding, marketPrice: data.quotes?.[holding.symbol] ?? holding.marketPrice })));
      setIrpQuoteUpdatedAt(new Date().toLocaleTimeString("ko-KR", { hour: "2-digit", minute: "2-digit" }));
    } catch { setNotice("IRP ETF 현재가를 불러오지 못했습니다. 마지막 확인 가격을 유지합니다."); }
  };
  ...
  useEffect(() => { if (isaHoldings.length > 0) void refreshIsaPrices(); }, [isaHoldings.length]);
  useEffect(() => { if (pensionHoldings.length > 0) void refreshPensionPrices(); }, [pensionHoldings.length]);
  useEffect(() => { if (irpHoldings.length > 0) void refreshIrpPrices(); }, [irpHoldings.length]);
  useEffect(() => {
    let mounted = true;
    const loadPortfolio = async () => {
      try {
        const response = await fetch("/api/portfolio");
        const data = await response.json() as { hasData?: boolean; state?: { accounts?: Account[]; imports?: ScreenshotImport[]; holdings?: Holding[]; coinHoldings?: CoinHolding[]; pensionHoldings?: PensionHolding[]; isaHoldings?: Holding[]; irpHoldings?: Holding[] } };
        ...
        if (data.hasData && data.state) {
          ...
          if (Array.isArray(data.state.isaHoldings)) setIsaHoldings(data.state.isaHoldings.map(holding => ({ ...holding, accountId: holding.accountId ?? 3 })));
          if (Array.isArray(data.state.irpHoldings)) setIrpHoldings(data.state.irpHoldings.map(holding => ({ ...holding, accountId: holding.accountId ?? 4 })));
        } else {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      void fetch("/api/portfolio", {
        ...
        body: JSON.stringify({ accounts, imports, holdings, coinHoldings, pensionHoldings, isaHoldings, irpHoldings }),
      }).then(response => {
  ...
  }, [accounts, imports, holdings, coinHoldings, pensionHoldings, isaHoldings, irpHoldings, hydrated]);
  const resetAccount = (account: Account) => {
    ...
    if (account.type === "국내 주식") setHoldings(current => current.filter(holding => holding.accountId !== account.id)); if (account.type === "코인") setCoinHoldings(current => current.filter(holding => holding.accountId !== account.id)); if (account.type === "연금저축") setPensionHoldings(current => current.filter(holding => holding.accountId !== account.id)); if (account.type === "ISA") setIsaHoldings(current => current.filter(holding => holding.accountId !== account.id)); if (account.type === "IRP") setIrpHoldings(current => current.filter(holding => holding.accountId !== account.id)); setImports(current => current.filter(item => item.accountId !== account.id));
  ...
  const deleteAccount = (account: Account) => {
    ...
    if (account.type === "국내 주식") setHoldings(current => current.filter(holding => holding.accountId !== account.id)); if (account.type === "코인") setCoinHoldings(current => current.filter(holding => holding.accountId !== account.id)); if (account.type === "연금저축") setPensionHoldings(current => current.filter(holding => holding.accountId !== account.id)); if (account.type === "ISA") setIsaHoldings(current => current.filter(holding => holding.accountId !== account.id)); if (account.type === "IRP") setIrpHoldings(current => current.filter(holding => holding.accountId !== account.id));
  ...
  return <main>
    ...
    <section className="accounts-section"><div className="panel-head"><div><p className="eyebrow">ACCOUNTS</p><h2>계좌별 자산</h2></div></div><p className="account-hint">계좌를 클릭하면 보유자산 상세와 현재가를 확인할 수 있습니다.</p><div className="account-table"><div className="table-heading"><span>계좌</span><span>자산 유형</span><span>평가금액</span><span>수익률</span><span>비중</span><span/></div>{accounts.map(account => <div className="account-item" key={account.id}><div className={`account-row ${expandedAccountId === account.id ? "expanded" : ""}`} role="button" tabIndex={0} onClick={() => setExpandedAccountId(current => current === account.id ? null : account.id)} onKeyDown={event => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); setExpandedAccountId(current => current === account.id ? null : account.id); } }}><div><span className={`account-icon ${account.color}`}>{account.type.slice(0, 1)}</span><span><b>{account.name}</b><small>{account.broker}</small></span></div><span className="type-chip">{account.type}</span><strong>{won.format(account.amount)}</strong><strong className={account.returnRate >= 0 ? "positive" : "negative"}>{percent(account.returnRate)}</strong><div className="weight"><i><em style={{ width: `${total > 0 ? account.amount / total * 100 : 0}%` }}/></i><span>{total > 0 ? (account.amount / total * 100).toFixed(1) : "0.0"}%</span></div><span className="row-actions"><i className="expand-arrow" aria-hidden="true">{expandedAccountId === account.id ? "⌃" : "⌄"}</i></span></div>{expandedAccountId === account.id && <AccountDetails account={account} holdings={holdings} coinHoldings={coinHoldings} pensionHoldings={pensionHoldings} isaHoldings={isaHoldings} irpHoldings={irpHoldings} quoteUpdatedAt={quoteUpdatedAt} coinQuoteUpdatedAt={coinQuoteUpdatedAt} isaQuoteUpdatedAt={isaQuoteUpdatedAt} pensionQuoteUpdatedAt={pensionQuoteUpdatedAt} irpQuoteUpdatedAt={irpQuoteUpdatedAt} refreshPrices={refreshPrices} refreshCoinPrices={refreshCoinPrices} refreshIsaPrices={refreshIsaPrices} refreshPensionPrices={refreshPensionPrices} refreshIrpPrices={refreshIrpPrices} />}</div>)}</div></section>
  </main>;
}
