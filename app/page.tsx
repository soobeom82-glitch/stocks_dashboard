...
function AccountDetails({ account, holdings, coinHoldings, pensionHoldings, isaHoldings, quoteUpdatedAt, coinQuoteUpdatedAt, refreshPrices, refreshCoinPrices }: { account: Account; holdings: Holding[]; coinHoldings: CoinHolding[]; pensionHoldings: PensionHolding[]; isaHoldings: Holding[]; quoteUpdatedAt: string; coinQuoteUpdatedAt: string; refreshPrices: () => Promise<void>; refreshCoinPrices: () => Promise<void> }) {
  ...
  const positions = isDomestic ? holdings.filter(holding => holding.accountId === account.id) : isCoin ? coinHoldings.filter(holding => holding.accountId === account.id) : isPension ? pensionHoldings.filter(holding => holding.accountId === account.id) : isIsa ? isaHoldings.filter(holding => holding.accountId === account.id) : [];
  const updatedAt = isDomestic ? quoteUpdatedAt : coinQuoteUpdatedAt;
  const refresh = isDomestic ? refreshPrices : refreshCoinPrices;

  if (!isDomestic && !isCoin && !isPension && !isIsa) return <div className="account-expanded empty-account-detail">등록된 보유자산이 없습니다.</div>;
  return <div className="account-expanded">
    <div className="detail-head"><div><p className="eyebrow">{isDomestic ? "DOMESTIC HOLDINGS" : isCoin ? "CRYPTO HOLDINGS" : isPension ? "PENSION HOLDINGS" : "ISA HOLDINGS"}</p><h3>{isDomestic ? "국내 주식 · 현재가 기준" : isCoin ? "코인 · 업비트 현재가 기준" : isPension ? "연금저축 · 마지막 등록 현재가 기준" : "ISA · 매수금액 기준"}</h3></div>{!isPension && !isIsa && <button className="text-button" onClick={event => { event.stopPropagation(); void refresh(); }}>현재가 새로고침 {updatedAt && `· ${updatedAt}`}</button>}</div>
    <p className="holdings-note">{isDomestic ? "등록된 보유 수량과 평단가를 기준으로 현재가 손익과 수익률을 계산합니다." : isCoin ? "업비트 KRW 마켓 현재가를 6시간마다 갱신하며, 버튼으로 즉시 다시 조회할 수 있습니다." : isPension ? "등록된 보유 수량과 평단가·현재가를 기준으로 손익과 수익률을 계산합니다." : "종목별 현재가가 보이는 화면을 등록하면 상세 손익을 정확히 갱신할 수 있습니다."}</p>
    {positions.length === 0 ? <div className="empty-holdings">등록된 {isCoin ? "코인 보유자산" : "보유 종목"}이 없습니다.</div> : <div className="holding-table"><div><span>{isCoin ? "코인" : "종목"}</span><span>보유 수량</span><span>평단가</span><span>현재가</span><span>평가손익</span><span>수익률</span></div>{positions.map(holding => { const profit = holding.quantity * (holding.fallbackPrice - holding.averagePrice); const rate = (holding.fallbackPrice / holding.averagePrice - 1) * 100; return <div key={holding.symbol}><b>{holding.name}<small>{isDomestic ? holding.symbol.replace(".KS", "") : holding.symbol}</small></b><span>{isCoin ? `${holding.quantity.toLocaleString("ko-KR", { maximumFractionDigits: 8 })} ${holding.symbol}` : `${holding.quantity}주`}</span><span>{won.format(holding.averagePrice)}</span><span>{won.format(holding.fallbackPrice)}</span><strong className={profit >= 0 ? "positive" : "negative"}>{profit >= 0 ? "+" : ""}{won.format(profit)}</strong><strong className={rate >= 0 ? "positive" : "negative"}>{percent(rate)}</strong></div>; })}</div>}
  </div>;
}

export default function Home() {
  const [accounts, setAccounts] = useState(initialAccounts); const [period, setPeriod] = useState(5); const [notice, setNotice] = useState(""); const [holdings, setHoldings] = useState<Holding[]>([]); const [coinHoldings, setCoinHoldings] = useState<CoinHolding[]>([]); const [pensionHoldings, setPensionHoldings] = useState<PensionHolding[]>([]); const [isaHoldings, setIsaHoldings] = useState<Holding[]>([]); const [quoteUpdatedAt, setQuoteUpdatedAt] = useState(""); const [coinQuoteUpdatedAt, setCoinQuoteUpdatedAt] = useState(""); const [expandedAccountId, setExpandedAccountId] = useState<number | null>(null); const [manageOpen, setManageOpen] = useState(false); const [newType, setNewType] = useState("국내 주식"); const [newName, setNewName] = useState(""); const [newBroker, setNewBroker] = useState(""); const [imports, setImports] = useState<ScreenshotImport[]>([]); const [hydrated, setHydrated] = useState(false); const syncErrorShown = useRef(false);
  ...
  useEffect(() => {
    ...
    const loadPortfolio = async () => {
      try {
        ...
        const data = await response.json() as { hasData?: boolean; state?: { accounts?: Account[]; imports?: ScreenshotImport[]; holdings?: Holding[]; coinHoldings?: CoinHolding[]; pensionHoldings?: PensionHolding[]; isaHoldings?: Holding[] } };
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      void fetch("/api/portfolio", {
        ...
        body: JSON.stringify({ accounts, imports, holdings, coinHoldings, pensionHoldings, isaHoldings }),
      }).then(response => {
  ...
  }, [accounts, imports, holdings, coinHoldings, pensionHoldings, isaHoldings, hydrated]);
  ...
  const deleteAccount = (account: Account) => {
    ...
    if (account.type === "국내 주식") setHoldings(current => current.filter(holding => holding.accountId !== account.id)); if (account.type === "코인") setCoinHoldings(current => current.filter(holding => holding.accountId !== account.id)); if (account.type === "연금저축") setPensionHoldings(current => current.filter(holding => holding.accountId !== account.id)); if (account.type === "ISA") setIsaHoldings(current => current.filter(holding => holding.accountId !== account.id));
