...
function TrendChart({ series, showLegend = true }: { series: TrendSeries[]; showLegend?: boolean }) {
  ...
  const cumulativeProfits = drawableSeries[0].snapshots.map((_, index) => drawableSeries.reduce((sum, item) => sum + (item.profits[index] ?? 0), 0));
  const axisProfit = Math.max(...cumulativeProfits.map(value => Math.abs(value)), 1) * 1.15;
