...
export default function Home() {
  ...
  const domesticTopThree = useMemo(() => {
    const grouped = new Map<string, { name: string; symbol: string; value: number; accountNames: Set<string> }>();
    assetDetailsByType["국내 주식"].holdings.forEach(({ account, holding, value }) => {
      const key = holding.symbol || holding.name;
      const current = grouped.get(key) ?? { name: holding.name, symbol: holding.symbol, value: 0, accountNames: new Set<string>() };
      current.value += value;
  ...
  return <main>
    ...
    <section className="panel domestic-top-panel"><div className="panel-head"><div><p className="eyebrow">DOMESTIC EQUITY TOP 3</p><h2>국내 주식 보유 비중 Top 3</h2></div><span className="domestic-total">국내 주식 내 비중</span></div><p className="account-hint">여러 계좌에 겹쳐 보유한 동일 종목은 합산합니다.</p>{domesticTopThree.items.length ? <div className="domestic-top-chart">{domesticTopThree.items.map((item, index) => <div className="domestic-top-row" key={item.symbol || item.name}><span className="domestic-rank">{index + 1}</span><div className="domestic-name"><b>{item.name}</b><small>{item.symbol} · {[...item.accountNames].join(", ")}</small></div><div className="domestic-bar"><i style={{ width: `${domesticTopThree.total ? item.value / domesticTopThree.total * 100 : 0}%` }} /></div><strong>{domesticTopThree.total ? (item.value / domesticTopThree.total * 100).toFixed(1) : "0.0"}%<small>{won.format(item.value)}</small></strong></div>)}</div> : <div className="empty-holdings">국내 주식 보유 종목을 등록하면 상위 3개 비중이 표시됩니다.</div>}</section>
