...
export default function Home() {
  ...
  return <main>
    ...
    <PerformancePanel title="통합 자산 추이" period={period} onPeriodChange={setPeriod} items={trendItems} selectedItems={selectedTrendItems} onToggleItem={toggleTrendItem} collapsed={portfolioTrendCollapsed} onToggleCollapsed={() => setPortfolioTrendCollapsed(current => !current)} pickerLabel="그래프에 표시할 계좌" />
    <PerformancePanel title="자산 유형별 추이" period={period} onPeriodChange={setPeriod} items={assetTrendItems} selectedItems={selectedAssetTrendItems} onToggleItem={toggleAssetTrendItem} collapsed={assetTrendCollapsed} onToggleCollapsed={() => setAssetTrendCollapsed(current => !current)} pickerLabel="그래프에 표시할 자산 유형" />
