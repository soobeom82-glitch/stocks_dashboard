...
export default function Home() {
  ...
  const [selectedTrendItems, setSelectedTrendItems] = useState<string[]>(["total"]);
  const [selectedAssetTrendItems, setSelectedAssetTrendItems] = useState<string[]>(["asset-total"]);
  ...
  return <main>
    ...
    <section className="content-grid"><article className="panel performance-panel"><div className="panel-head"><div><p className="eyebrow">PERFORMANCE</p><h2>통합 자산 추이</h2></div></div><div className="periods" role="tablist">{reports.map(item => <button key={item} role="tab" aria-selected={period === item} className={period === item ? "selected" : ""} onClick={() => setPeriod(item)}>{item}</button>)}</div><div className="trend-item-picker" aria-label="그래프 표시 항목">{trendItems.map(item => <button key={item.id} className={selectedTrendItems.includes(item.id) ? "selected" : ""} onClick={() => toggleTrendItem(item.id)} aria-pressed={selectedTrendItems.includes(item.id)}><i style={{ background: item.color }} />{item.name}</button>)}</div><div className="report-value"><div><span>{reportLabels[period]} · 선택 항목 {visibleTrendItems.length}개</span><strong>{reportChange === null ? "비교 기준 생성 중" : percent(reportChange)}</strong></div><span className="report-description">선택한 항목은 하나의 그래프에 색상별 선으로 겹쳐 표시합니다</span></div><div className="trend-series-list">{visibleTrendItems.length ? <TrendChart series={visibleTrendItems} /> : <div className="empty-holdings">그래프로 볼 계좌를 하나 이상 선택해 주세요.</div>}</div></article></section>
    <section className="content-grid"><article className="panel performance-panel"><div className="panel-head"><div><p className="eyebrow">ASSET PERFORMANCE</p><h2>자산 유형별 추이</h2></div></div><div className="periods" role="tablist">{reports.map(item => <button key={item} role="tab" aria-selected={period === item} className={period === item ? "selected" : ""} onClick={() => setPeriod(item)}>{item}</button>)}</div><div className="trend-item-picker" aria-label="자산 유형 그래프 표시 항목">{assetTrendItems.map(item => <button key={item.id} className={selectedAssetTrendItems.includes(item.id) ? "selected" : ""} onClick={() => toggleAssetTrendItem(item.id)} aria-pressed={selectedAssetTrendItems.includes(item.id)}><i style={{ background: item.color }} />{item.name}</button>)}</div><div className="report-value"><div><span>{reportLabels[period]} · 선택 항목 {visibleAssetTrendItems.length}개</span><strong>{assetReportChange === null ? "비교 기준 생성 중" : percent(assetReportChange)}</strong></div><span className="report-description">선택한 자산 유형은 하나의 그래프에 색상별 선으로 겹쳐 표시합니다</span></div><div className="trend-series-list">{visibleAssetTrendItems.length ? <TrendChart series={visibleAssetTrendItems} /> : <div className="empty-holdings">그래프로 볼 자산 유형을 하나 이상 선택해 주세요.</div>}</div></article></section>
