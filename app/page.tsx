...
function AccountDetails({ account, holdings, coinHoldings, pensionHoldings, isaHoldings, irpHoldings, quoteUpdatedAt, coinQuoteUpdatedAt, refreshPrices, refreshCoinPrices }: { account: Account; holdings: Holding[]; coinHoldings: CoinHolding[]; pensionHoldings: PensionHolding[]; isaHoldings: Holding[]; irpHoldings: Holding[]; quoteUpdatedAt: string; coinQuoteUpdatedAt: string; refreshPrices: () => Promise<void>; refreshCoinPrices: () => Promise<void> }) {
  ...
  return <div className="account-expanded">
    <div className="detail-head"><div><p className="eyebrow">{isDomestic ? "DOMESTIC HOLDINGS" : isCoin ? "CRYPTO HOLDINGS" : isPension ? "PENSION HOLDINGS" : isIsa ? "ISA HOLDINGS" : "IRP HOLDINGS"}</p><h3>{isDomestic ? "국내 주식 · 현재가 기준" : isCoin ? "코인 · 업비트 현재가 기준" : isPension ? "연금저축 · 마지막 등록 현재가 기준" : isIsa ? "ISA · 매수금액 기준" : "IRP · 투자상품 및 현금성 자산"}</h3></div>{!isPension && !isIsa && !isIrp && <button className="text-button" onClick={event => { event.stopPropagation(); void refresh(); }}>현재가 새로고침 {updatedAt && `· ${updatedAt}`}</button>}</div>
    <p className="holdings-note">{isDomestic ? "등록된 보유 수량과 평단가를 기준으로 현재가 손익과 수익률을 계산합니다." : isCoin ? "업비트 KRW 마켓 현재가를 6시간마다 갱신하며, 버튼으로 즉시 다시 조회할 수 있습니다." : isPension ? "등록된 보유 수량과 평단가·현재가를 기준으로 손익과 수익률을 계산합니다." : isIsa ? "이 화면에는 종목별 현재가가 없어 매수금액과 계좌 전체 평가손익만 반영했습니다. 임시 티커와 추정 현재가는 표시하지 않습니다." : "ETF·주식과 예금·보험·디폴트옵션 등 현금성 금융상품을 구분해 보여줍니다."}</p>
    {positions.length === 0 ? <div className="empty-holdings">등록된 {isCoin ? "코인 보유자산" : "보유 종목"}이 없습니다.</div> : positionGroups.map(group => <div className="holding-group" key={group.assetClass}>{isIrp && <h4>{group.assetClass}</h4>}<div className="holding-table"><div><span>{isCoin ? "코인" : isIrp ? "상품" : "종목"}</span><span>보유 수량</span><span>매입금액</span><span>{isIsa ? "종목별 현재가" : "평가금액"}</span><span>{isIsa ? "계좌 손익" : "평가손익"}</span><span>{isIsa ? "계좌 수익률" : "수익률"}</span></div>{group.positions.map(holding => { const profit = holding.quantity * (holding.fallbackPrice - holding.averagePrice); const rate = (holding.fallbackPrice / holding.averagePrice - 1) * 100; return <div key={`${holding.symbol}-${holding.name}`}><b>{holding.name}{!isIsa && <small>{isDomestic ? holding.symbol.replace(".KS", "") : holding.symbol}</small>}</b><span>{isCoin ? `${holding.quantity.toLocaleString("ko-KR", { maximumFractionDigits: 8 })} ${holding.symbol}` : `${holding.quantity}${holding.unit ?? "주"}`}</span><span>{won.format(holding.quantity * holding.averagePrice)}</span>{isIsa ? <><span className="muted-cell">미연결</span><strong className="muted-cell">-</strong><strong className="muted-cell">-</strong></> : <><span>{won.format(holding.quantity * holding.fallbackPrice)}</span><strong className={profit >= 0 ? "positive" : "negative"}>{profit >= 0 ? "+" : ""}{won.format(profit)}</strong><strong className={rate >= 0 ? "positive" : "negative"}>{percent(rate)}</strong></>}</div>; })}</div></div>)}
  </div>;
}

export default function Home() {
  const [accounts, setAccounts] = useState(initialAccounts); const [period, setPeriod] = useState(5); const [notice, setNotice] = useState(""); const [holdings, setHoldings] = useState<Holding[]>([]); const [coinHoldings, setCoinHoldings] = useState<CoinHolding[]>([]); const [pensionHoldings, setPensionHoldings] = useState<PensionHolding[]>([]); const [isaHoldings, setIsaHoldings] = useState<Holding[]>([]); const [irpHoldings, setIrpHoldings] = useState<Holding[]>([]); const [quoteUpdatedAt, setQuoteUpdatedAt] = useState(""); const [coinQuoteUpdatedAt, setCoinQuoteUpdatedAt] = useState(""); const [expandedAccountId, setExpandedAccountId] = useState<number | null>(null); const [manageOpen, setManageOpen] = useState(false); const [newType, setNewType] = useState("국내 주식"); const [newName, setNewName] = useState(""); const [newBroker, setNewBroker] = useState(""); const [imports, setImports] = useState<ScreenshotImport[]>([]); const [hydrated, setHydrated] = useState(false); const syncErrorShown = useRef(false);
